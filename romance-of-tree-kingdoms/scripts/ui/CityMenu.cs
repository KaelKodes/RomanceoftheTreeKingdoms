...
public partial class CityMenu : Control
{
    ...
    private void DoTrain(string stat)
    {
        ...
        else
        {
            ...
            {
                // (int OfficerId, string Name, int StatValue, int Cost, int Relation, bool IsAvailable)
                var btn = new Button();
                string availability = m.IsAvailable ? "" : " [Exhausted Today]";
                btn.Text = $"{m.Name} (Stat: {m.StatValue}, Cost: {m.Cost}g){availability}";

                if (!m.IsAvailable)
                {
                    btn.Disabled = true;
                    btn.Modulate = Colors.Gray;
                }
                else
                {
